...
img_size = 28
onehot = np.eye(10)
temp_z_ = np.random.uniform(-1, 1, (10, 1, 1, 100))
fixed_z_ = temp_z_
...
lr = tf.train.exponential_decay(0.00025, global_step, 500, 0.94, staircase=True)
# load MNIST
...
yqd = np.load('input/data/y.npy')
xqd = xqd / 255
aa = np.concatenate((xqd,yqd),axis=1)
np.random.shuffle(aa)
xqd = aa[:,0:784]
yqd = aa[:,784:794]
xqd = xqd.reshape([60000,28,28])
xqd = xqd.reshape([60000,28,28,1])
train_set = (xqd - 0.5) / 0.5
train_label = yqd

# results save folder
root = 'input/MNIST_cDCGAN_results/'
model = 'MNIST_cDCGAN_'
...
save_path = saver.save(sess, root+"save_net.ckpt")
sess.close()